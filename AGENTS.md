...
### 4. 破壊的変更の禁止 (Prohibition of Destructive Actions) - CRITICAL
- **推測によるファイル削除・変更の絶対禁止**: 既存のファイル（環境構築ファイル、バッチファイル、ログファイルなどを含む）を、「不要そうだから」という推測や独自の判断で絶対に削除・リネームしてはならない。
- 何らかのクリーンアップが必要だと判断した場合は、**必ず事前にユーザーへ対象ファイルのリストを提示し、明確な許可を得ること**。勝手な判断による削除が発覚した場合は深刻なエラーとみなす。

### 5. Knowledge Sync（記憶の共有化）
- 迷ったら推測する前に `docs/` を読む。
