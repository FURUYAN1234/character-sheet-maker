...
![Version](https://img.shields.io/badge/version-1.3.10-4f46e5)
...
![Output](https://img.shields.io/badge/output-1024px%20portrait%20PNG-10b981)
...
The current public line is **v1.3.10**. This version is a browser-based React/Vite app with session-only API keys, dual Gemini/OpenAI routing, A/B comparison, parameter locks, deterministic Japanese profile typesetting, direct named prompt-file downloads, preserved 1024x1536 artwork, and provenance watermarking.

現行公開系統は **v1.3.10** です。ブラウザ上で動作する React/Vite アプリで、セッション限定APIキー、Gemini/OpenAIの切り替え、A/B比較、項目ロック、日本語プロフィールの決定的な文字合成、名前付きプロンプトファイルの直接ダウンロード、1024x1536のイラスト維持、来歴ウォーターマークを備えています。

| Area / 領域 | Current behavior / 現行挙動 |
|---|---|
| App version / バージョン | `1.3.10`, displayed in the API gate, header, footer, and watermark. |
...
| Output canvas / 出力キャンバス | A Japanese profile header is typeset above the preserved `1024x1536` portrait artwork. Final PNG width is 1024px; total height grows to fit longer profile text. |
...
7. **Typeset, normalize, and compare / 文字合成・正規化・比較**
   The app adds the exact profile fields in a Japanese header above the preserved 1024x1536 portrait artwork, then applies the version watermark. The final PNG is 1024px wide and grows vertically to fit the profile. A/B mode allows two slots to be compared side by side.
...
* Artwork area: `1024x1536`
* Final PNG: 1024px wide; height is the 1536px artwork plus a profile header that grows when entries wrap.
* Aspect ratio: portrait; it varies with the profile header height.
* File type: PNG
* Profile text: exact Japanese character information is rendered by the app above the illustration.
* Watermark: `Generated by Super FURU AI Character Sheet v1.3.10`
