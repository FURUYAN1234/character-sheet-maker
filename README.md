...
> **[Nano Banana 2 and ChatGPT Images 2.0 Powered Super AI 4-koma System](https://github.com/FURUYAN1234/nano-banana-pro/blob/main/README.md) 連携対応**
> 生成されたキャラクターシートを [Nano Banana 2 and ChatGPT Images 2.0 Powered Super AI 4-koma System](https://github.com/FURUYAN1234/nano-banana-pro) の OCR で読み取り、マンガ制作の幅を広げることができます。
...
[Nano Banana 2 and ChatGPT Images 2.0 Powered Super AI 4-koma System](https://github.com/FURUYAN1234/nano-banana-pro/blob/main/README.md) で生成するマンガのキャラクター品質と多様性を飛躍的に向上させるための、専用キャラクター設計ツールとして開発されました。
...
- **Gemini**: `gemini-3.5-flash` 系のテキスト生成と `gemini-3.1-flash-image` による縦長キャラクターシート生成
- **OpenAI**: `gpt-4.1` 系のテキスト生成と `gpt-image-2` による高品質イラスト生成
...
**【OpenAI エンジン (ChatGPT)】**
**テキスト生成 / Text Generation**: gpt-4.1 -> gpt-4.1-mini -> gpt-4.1-nano -> gpt-4o
**画像生成 / Image Generation**: gpt-image-2 (1024x1792 / high / output_format=png / 600s timeout)
...
- **[Fix]** OpenAI画像生成を現行の `gpt-image-2` 仕様に合わせ、`output_format: "png"`、600秒タイムアウト、32,000文字プロンプトガード、PNG MIME返却に対応。
- **[Fix]** 画像MIME情報をキャンバス透かし処理前に保持し、プロバイダ返却形式との不一致を防止。
- **[Fix]** 構文を壊していた文字化け文字列を除去するためOpenAIクライアントを再構築し、公開API互換性は維持。
...
All generations are performed through the **official Google Gemini API** or **official OpenAI API** selected by the user. This system adheres to each provider's terms and safety policies.
本システムはユーザーが選択したGoogle公式Gemini APIまたはOpenAI公式APIを介して動作し、各提供元の利用規約および安全ポリシーを遵守します。
